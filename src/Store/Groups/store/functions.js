...
export const fetchCurrentPageGroups = ()=> (dispatch, getState) => {
...
export const FetchMany = (pageNo, type) => async (dispatch) => {
    ...
    try {
        ...
        if (response == null) {
            // console.log("Empty response-> grp-func-98")
            dispatch({type: types.LAST_PAGE_REACHED});


            return
        }
        if (response.length < GroupsLimit) {
...
export const Create = (formData) => async (dispatch) => {
    ...
    try {
        ...
        dispatch(fetchCurrentPageGroups());
        // return true
    } catch (err) {
