...
import * as SELECTORS from "./genres.selectors";

import * as ACTIONS from "./genres.actions";
import {GenresLimit} from "Constants/constants";
...
export const fetchCurrentPageGenres = ()=> (dispatch, getState) => {
...
export const FetchMany = (pageNo, type) => async (dispatch) => {
    ...
    try {
        ...
        if (response.length < GenresLimit) {
            dispatch({type: types.LAST_PAGE_REACHED})

        }
        dispatch(ACTIONS.setPage(type, pageNo ))

        dispatch(ACTIONS.receivedMany(response, pageNo))
...
export const Create = (formData) => async (dispatch) => {
    ...
    try {
        ...
        dispatch(fetchCurrentPageGenres());
        // return true
    } catch (err) {
