...
import  * as SELECTORS from "./category_selectors";

import * as ACTIONS from "./category_actions";
import shop from "../../../Data/shop"
import {CategoryLimit} from "../../../Constants/constants";
...
export const fetchCurrentPageCategories = ()=> (dispatch, getState) => {
...
export const FetchMany = (pageNo, type) => async (dispatch) => {
    ...
    try {
        ...
        if (response.length < CategoryLimit) {
            dispatch({type: types.LAST_PAGE_REACHED})
...
export const Create = (formData) => async (dispatch) => {
    ...
    try {
        ...
        dispatch(fetchCurrentPageCategories());
        // return true
    } catch (err) {
